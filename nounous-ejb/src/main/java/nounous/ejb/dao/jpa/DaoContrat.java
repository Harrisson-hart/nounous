...
public class DaoContrat implements IDaoContrat {
	...
	public List<Contrat> listerTout() {
		em.clear();
		var jpql = "SELECT c FROM Contrat c";
	...
	@Override
	@TransactionAttribute( NOT_SUPPORTED )
	public List<Contrat> listerPourNounou( int idCompte) {
		em.clear();
		var jpql = "SELECT c FROM Contrat c Join c.Compte co WHERE co.id=:idCompte";
		var query = em.createQuery( jpql, Contrat.class );
